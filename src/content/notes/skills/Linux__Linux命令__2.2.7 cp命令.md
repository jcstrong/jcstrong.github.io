...
tags: ["Linux", "运维"]
featured: false
source: "Linux/Linux命令/2.2.7 cp命令.md"
updated: 2021-03-03
readingTime: 4
summary: "toc cp命令用来复制文件或者目录，是Linux系统中最常用的命令之一。一般情况下，shell会设置一个别名，在命令行下复制文件时，如果目标文件已经存在，就会询问是否覆盖，不管你是否使用i参数。但是如果是在shell脚本中执行cp时，没有..."
---
[toc]
...
说明：
那个 log_link.log 是由 -s 的参数造成的，建立的是一个快捷方式，所以您会看到在文件的最右边，会显示这个文件是连结到哪里去的！