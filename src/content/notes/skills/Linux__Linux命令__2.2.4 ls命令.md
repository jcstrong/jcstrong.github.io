...
tags: ["Linux", "运维"]
featured: false
source: "Linux/Linux命令/2.2.4 ls命令.md"
updated: 2022-06-06
readingTime: 6
summary: "toc ls命令是linux下最常用的命令。ls命令就是list的缩写缺省下ls用来打印出当前目录的清单如果ls指定其他目录那么就会显示指定目录里的文件及文件夹清单。 通过ls 命令不仅可以查看linux文件夹包含的文件而且可以查看..."
---
[toc]
...
### 实例1：列出/home/peidachang文件夹下的所有文件和目录的详细资料
...
### 实例2：列出当前目录中所有以“t”开头的目录的详细内容，可以使用如下命令：
...
### 实例4：列出目前工作目录下所有名称是s 开头的档案，愈新的排愈后面，可以使用如下命令：
...
### 实例5：列出目前工作目录下所有档案及目录;目录于名称后加"/", 可执行档于名称后加"*"
...
### 实例8：列出当前目录下的所有文件（包括隐藏文件）的绝对路径， 对目录不做递归
...
### 实例9：递归列出当前目录下的所有文件（包括隐藏文件）的绝对路径
...
### 列出当前目录，可见文件详细信息。列出当前目录，可见文件详细信息。

### 列出当前文件以T开头的目录详细内容。

### 列出所有文件包括隐藏文件的详细信息。

### 列出目前工作目录中所有 名称是S开头的文件最新的文件排在最后。

### 将bin目录下所有目录及文件详细资料列出。

### 按修改时间列出文件和文件夹详细信息。

### 将/home目录以下所有目录 列出当前目录中，所有1T开头的目录的相信。及文件详细资料列出。

### 列出目前工作目录下的所有文件及目录目录与名称，后面加 /， 可执行文档于名称后加 * 号。

### 增强对文件大小易读性，以更易读的字节单位（K、M等）显示信息。

![](https://jcstrong.github.io/img/20210207033552.png)

![](https://jcstrong.github.io/img/20210207032741.png)