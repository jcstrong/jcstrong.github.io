---
title: "2.2.9.1 find命令"
category: skills
tags: ["Linux", "运维"]
featured: false
source: "Linux/Linux命令/2.2.9.1 find命令.md"
updated: 2021-02-07
readingTime: 18
summary: "toc  一、主要内容 1. 用文件名查找文件 2. 用文件名查找文件，忽略大小写用文件名查找文件，忽略大小写 3. 使用mindepth和maxdepth限定搜索指定目录的深度 4. 在find命令查找到的文件上执行命令 5. 相反匹配 ..."
---
[toc]
...
1. 用文件名查找文件
2. 用文件名查找文件，忽略大小写用文件名查找文件，忽略大小写
...
[touchMybashProgram.sh](https://touchmybashprogram.sh/)
...
backup [MybashProgram.sh](https://mybashprogram.sh/) MyCProgram.c

create_sample_files.sh mycprogram.c Program.c

./backup:

[MybashProgram.sh](https://mybashprogram.sh/) mycprogram.c MyCProgram.c Program.c
...
（这里的改变更第1个例子的更改文件内容时间是不同概念，这里是更改的是文件inode的数据，比如文件的权限，所属人等等信息）
...
（如果上面的n为-n形式，则表示n分钟/天之内，n为+n则表示n分钟/天之前）
...
上面的双引号是为了防止文件名中出现的空格，不加的话会有问题。[然后你把这个shell脚本保存为mv.sh](https://xn--shellmv-q33kze35jqubx52al7wu6wq2pu11bfq8ciqzc.sh/)，你可以像下面这样使用find命令了
