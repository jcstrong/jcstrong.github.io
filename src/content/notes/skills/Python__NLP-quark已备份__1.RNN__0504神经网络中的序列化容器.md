---
title: "0504神经网络中的序列化容器"
category: skills
tags: ["RNN", "NLP"]
featured: false
source: "Python/NLP-quark已备份/1.RNN/0504神经网络中的序列化容器.md"
updated: 2022-04-21
readingTime: 3
summary: "Pytorch中的序列化容器\r \r  目标\r \r 1. 知道梯度消失和梯度爆炸的原理和解决方法\r 2. 能够使用nn.Sequential完成模型的搭建\r 3. 知道nn.BatchNorm1d的使用方法\r 4. 知道nn.Dropout的..."
---
# Pytorch中的序列化容器
...
在使用pytorch中的序列化容器之前，我们先来了解一下常见的梯度消失和梯度爆炸的问题

### 1.1 梯度消失

假设我们有四层极简神经网络：每层只有一个神经元

![](https://s2.loli.net/2022/04/21/6oGp7tMRbwif4jD.png)

$获取w1的梯度有：▽w1 = x1*f(a1)’*w2*f(b1)’*w3*▽out$

假设我们使用sigmoid激活函数，即f为sigmoid函数，sigmoid的导数如下图

![](https://s2.loli.net/2022/04/21/sU6TDiVM59OrREm.png)
...
1. **替换易训练神经元**
   ...
   ![](https://s2.loli.net/2022/04/21/Y6XewFsWV78GqZH.png)
