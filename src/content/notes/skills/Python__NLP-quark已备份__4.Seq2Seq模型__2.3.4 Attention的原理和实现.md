---
title: "2.3.4 Attention的原理和实现"
category: skills
tags: ["Seq2Seq", "Attention", "NLP"]
featured: false
source: "Python/NLP-quark已备份/4.Seq2Seq模型/2.3.4 Attention的原理和实现.md"
updated: 2023-03-05
readingTime: 11
summary: "Attention的原理和实现\r \r  目标\r \r 1. 知道Attention的作用\r 2. 知道Attention的实现机制\r 3. 能够使用代码完成Attention代码的编写\r \r \r \r  1. Attention的介绍\r \r 在..."
---
# Attention的原理和实现
...
![](https://chenjun-xs.oss-cn-hangzhou.aliyuncs.com/attention1.png)
...
2. 这个$z_o$会和encoder第一个time step的output进行match操作（或者是socre操作），得到$\alpha_0^1$ ，这里的match可以使很多中操作，比如：
   ...
   - 是一个神经网络，输入为z和h
   - 或者$\alpha = h^T W z$等
   - ![](https://chenjun-xs.oss-cn-hangzhou.aliyuncs.com/Attention2.png)
...
4. 之后把所有的softmax之后的结果和原来encoder的输出$h_i$进行相加求和得到$c^0$
   ...
   - ![](https://chenjun-xs.oss-cn-hangzhou.aliyuncs.com/Attention3.png)

5. 得到$c^0$之后，把它作为decoder的input，同和传入初始化的$z^0$，得到第一个time step的输出和hidden_state（$Z^1$）

   - ![](https://chenjun-xs.oss-cn-hangzhou.aliyuncs.com/Attention4.png)
...
7. 再把$c^1$作为decoder的input，和$Z^1$作为输入得到下一个输出，如此循环,只到最终decoder的output为终止符
   ...
   - ![](https://chenjun-xs.oss-cn-hangzhou.aliyuncs.com/Attention5.png)
...
![](https://chenjun-xs.oss-cn-hangzhou.aliyuncs.com/soft-hard%20attention.jpg)
...
1. attention的计算数据和位置
   ...
   1. `Bahdanau Attention`会使用`前一次的隐藏`状态来计算attention weight，所以我们会在代码中的GRU之前使用attention的操作，同时会把attention的结果和word embedding的结果进行concat，作为GRU的输出(参考的是pytorch Toritul)。Bahdanau使用的是双向的GRU，会使用正反的encoder的output的concat的结果作为encoder output,如下图所示
      - ![](https://chenjun-xs.oss-cn-hangzhou.aliyuncs.com/Bahdanau.png)
   2. `Luong Attenion`使用的是`当前一次的decoder的output`来计算得到attention weight，所以在代码中会在GRU的后面进行attention的操作，同时会把`context vector`和gru的结果进行concat的操作，最终的output。Luong使用的是多层GRU，只会使用最后一层的输出(encoder output)
      - ![](https://chenjun-xs.oss-cn-hangzhou.aliyuncs.com/Luong.png)
...
2. 计算attention weights的方法不同
   ...
   2. `Luong Attenion`整体比`Bahdanau Attention`更加简单，他使用了三种方法来计算得到权重
      ...
      4. ![](https://chenjun-xs.oss-cn-hangzhou.aliyuncs.com/scores.png)
...
![](https://chenjun-xs.oss-cn-hangzhou.aliyuncs.com/attention6.png)
