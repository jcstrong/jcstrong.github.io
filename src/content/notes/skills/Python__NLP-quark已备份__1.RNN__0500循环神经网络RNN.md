...
tags: ["RNN", "NLP"]
featured: false
source: "Python/NLP-quark已备份/1.RNN/0500循环神经网络RNN.md"
updated: 2023-03-05
readingTime: 4
summary: "toc\r \r  循环神经网络和自然语言处理介绍\r \r  目标\r \r 1. 知道token和tokenization\r 2. 知道Ngram的概念和作用\r 3. 知道文本向量化表示的方法\r \r \r \r \r \r  1. 文本的tokenizat..."
---
[toc]
...
==`tokenization`就是通常所说的分词==，分出的==每一个词语我们把它称为`token`==。
...
`N-gram`一组一组的词语，其中的`N`表示==能够被一起使用的词的数量==

N-gram，特征个数就是 $总长度-(n-1)$；
...
# N-gram 中n=2时，特征个数就是总长度-1；
In [61]: [cuted[i:i+2] for i in range(len(cuted)-1)] 
Out[61]:[['深度', '学习'],
 ['学习', '（'],
 ['（', '英语'],
 ['英语', '：'],
...
word embedding是深度学习中表示文本常用的一种方法。和one-hot编码不同，word embedding使用了==浮点型的稠密矩阵来表示token==。

根据词典的大小，我们的向量通常使用不同的维度，例如100,256,300等。其中向量中的每一个值是一个参数，其==初始值是随机生成的==，之后会在训练的过程中进行学习而获得。
...
![word_embedding](https://chenjun-xs.oss-cn-hangzhou.aliyuncs.com/word_embedding.png)
...
`[batch_size, seq_len] ---> [batc_size, seq_len, embedding_dim]`

![截屏2022-04-20 23.01.17](https://s2.loli.net/2022/04/20/PJ2pdgqjwfZ41Km.png)
