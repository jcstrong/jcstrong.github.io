...
Django默认初始配置使用**sqlite**数据库。在settings.py中保存了数据库的连接配置信息
...
class BookInfo(models.Model):
    ...
    class Meta:
        ...
        verbose_name = 'admin'  # 修改后台admin的显示信息的配置
...
class PeopleInfo(models.Model):
    ...
    class Meta:
        db_table = 'peopleinfo'        # Meta改当前类的表名，默认是‘子应用_类名’
        verbose_name = '人物信息'				# 后台别名
...
- 定义属性时需要指定字段类型，通过字段类型的参数指定选项，语法如下：
  ...
  ```python
    #属性名				字段类型		选项  
    name = models.CharField(max_length=10, unique=True, null=True, verbose_name='名字')    
...
https://docs.djangoproject.com/zh-hans/3.2/intro/tutorial02/
...
查看对数据库的操作记录。 mysql日志文件默认没有产生，需要做如下配置：
...
>  属性名称和比较运算符间使用**两个下划线**，所以属性名不能包括多个下划线
...
```python
>>> BookInfo.objects.filter(id=1)		# 返回的是个列表
<QuerySet [<BookInfo: 射雕英雄传>]>
```

```python
>>> BookInfo.objects.get(id=1)			# 返回的是一个对象
<BookInfo: 射雕英雄传>
```
...
##### **startswith、endswith：以指定值开头或结尾。** 查询书名以'部'结尾的图书
...
> 以上运算符都区分大小写，在这些**运算符前加上i表示不区分大小写**，如iexact、icontains、istartswith、iendswith.

#### 空查询

##### **isnull：是否为null。** 查询书名为空的图书。
...
##### **gt大于 (greater then)**  查询编号大于3的图书

```python
BookInfo.objects.filter(id__gt=3)
```

##### exclude()过滤器	查询编号不等于3的图书
...
## [使用 F() 避免竞争条件](https://docs.djangoproject.com/zh-hans/3.2/ref/models/expressions/#avoiding-race-conditions-using-f)
...
# F() 还可以与 update() 一起用于对象实例的 QuerySets
...
多个过滤器逐个调用表示逻辑与关系，同sql语句中**where部分的and关键字**。
...
- 使用`objects.aggregate()`过滤器调用聚合函数。
...
# 降序
 BookInfo.objects.all().order_by('-readcount')
<QuerySet [<BookInfo: 雪山飞狐>, <BookInfo: 天龙八部>, <BookInfo: 笑傲江湖>, <BookInfo: 射雕
```
...
本例中book与people关系为：一对多；book中没有people的字段，但是people中有book外键
...
这样未经过存储，不会产生缓存，每次生成列表的时候都进行一次查询
...
经过books变量的存储后，只有第一次生成列表时进行了查询，之后每次都是通过缓存获得的
...
如果没有数据，[0]引发IndexError异常，[0:1].get()引发DoesNotExist异常。
...
[Django官方文档](https://docs.djangoproject.com/en/1.11/topics/pagination/)
